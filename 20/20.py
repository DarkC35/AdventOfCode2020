...
def count_arrangements_not_efficient(adapters, current_jolts):
    compatible_adapter_indexes = [
        i for i, x in enumerate(adapters[0:3]) if x <= current_jolts + 3
    ]
    if len(compatible_adapter_indexes) == 0:
        return 1
    count = 0
    for index in compatible_adapter_indexes:
        count += count_arrangements_not_efficient(
            adapters[index+1:], adapters[index])
    return count


# solution with caching! (thx Dani & Ginzi)
def count_arrangements(adapters, current_jolts, cache):
    compatible_adapter_indexes = [i for i, x in enumerate(
        adapters[0:3]) if x <= current_jolts + 3]
    if len(compatible_adapter_indexes) == 0:
        return 1
